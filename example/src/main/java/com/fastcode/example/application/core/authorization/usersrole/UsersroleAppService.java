	...
	protected BooleanBuilder searchKeyValuePair(QUsersrole usersrole, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
		    ...
		    if(details.getKey().replace("%20","").trim().equals("users")) {
				if(details.getValue().getOperator().equals("contains")) {
					builder.and(usersrole.users.firstname.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
				} else if(details.getValue().getOperator().equals("equals")) {
					builder.and(usersrole.users.firstname.eq(details.getValue().getSearchValue()));
				} else if(details.getValue().getOperator().equals("notEqual")) {
					builder.and(usersrole.users.firstname.ne(details.getValue().getSearchValue()));
				}
			}
		}
		...
		if(joinCol != null && joinCol.getKey().equals("users")) {
		    builder.and(usersrole.users.firstname.eq(joinCol.getValue()));
        }
        }
		return builder;
	}
