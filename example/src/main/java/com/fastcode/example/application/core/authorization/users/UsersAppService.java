...
public class UsersAppService implements IUsersAppService {
	...
	public Users getUsers() {
		...
		return _usersRepository.findByFirstnameIgnoreCase(SecurityUtils.getCurrentUserLogin().orElse(null));
	}
	
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public FindUsersByFirstnameOutput findByFirstname(String firstname) {

		Users foundUsers = _usersRepository.findByFirstnameIgnoreCase(firstname);
		if (foundUsers == null) {
			return null;
		}

		return  mapper.usersToFindUsersByFirstnameOutput(foundUsers);
	}
	
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public FindUsersByFirstnameOutput findByEmailaddress(String emailAddress) {
