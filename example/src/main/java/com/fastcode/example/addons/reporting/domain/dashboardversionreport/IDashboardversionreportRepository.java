...
public interface IDashboardversionreportRepository extends JpaRepository<Dashboardversionreport, DashboardversionreportId>,QuerydslPredicateExecutor<Dashboardversionreport> {
	...
	@Query("select r from Dashboardversionreport r where r.dashboardversionEntity.dashboardId = ?1 and r.dashboardversionEntity.dashboardVersion =?2 and r.dashboardversionEntity.userId=?3 ")
	List<Dashboardversionreport> findByDashboardIdAndVersionAndUsersId(Long dashboardId, String version, Long userId);
    
	@Query("select r from Dashboardversionreport r where r.dashboardversionEntity.dashboardId = ?1 and r.dashboardversionEntity.dashboardVersion =?2 and r.dashboardversionEntity.userId=?3  ORDER BY r.orderId DESC")
	List<Dashboardversionreport> findByDashboardIdAndVersionAndUsersIdInDesc(Long id, String version, Long userId);

    @Query("select r from Dashboardversionreport r where r.report.id = ?1 and r.dashboardversionEntity.userId=?2  and r.dashboardversionEntity.dashboardVersion =?3")
    ...
    @Query("select r from Dashboardversionreport r where r.dashboardversionEntity.dashboardId = ?1 and r.report.id = ?2 and r.dashboardversionEntity.userId != ?3  and r.dashboardversionEntity.dashboardVersion =?4")
