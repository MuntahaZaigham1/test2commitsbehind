...
public class UsersController {
	...
	public ResponseEntity<CreateUsersOutput> create( @RequestBody @Valid CreateUsersInput users) {
		FindUsersByFirstnameOutput foundUsers = _usersAppService.findByFirstname(users.getFirstname());

	        if (foundUsers != null) {
	            logHelper.getLogger().error("There already exists a users with a Firstname=%s", users.getFirstname());
	            throw new EntityExistsException(
	                    String.format("There already exists a users with Firstname =%s", users.getFirstname()));
	        }
	...
	public ResponseEntity<UsersProfile> updateProfile(@RequestBody @Valid UsersProfile usersProfile) {
        ...
        FindUsersByFirstnameOutput usersOutput;
		usersOutput = _usersAppService.findByFirstname(users.getFirstname());
		if(usersOutput != null &&
		usersOutput.getId() != users.getId() )
		{
			logHelper.getLogger().error("There already exists a users with user name=%s", users.getFirstname());
			throw new EntityExistsException(
					String.format("There already exists a users with user name=%s", users.getFirstname()));
		}
	...
	public ResponseEntity<UpdateUsersOutput> update(@PathVariable String id,  @RequestBody @Valid UpdateUsersInput users) {
	    ...
	    if(currentUsers.getIsactive() && !users.getIsactive()) {
           _jwtAppService.deleteAllUserTokens(currentUsers.getFirstname());
        }
