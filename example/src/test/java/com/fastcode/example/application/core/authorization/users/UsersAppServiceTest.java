...
public class UsersAppServiceTest {
	...
	public void findUsersByName_NameIsNotNullAndUsersDoesNotExist_ReturnNull() {
		Mockito.when(_usersRepository.findByFirstnameIgnoreCase(anyString())).thenReturn(null);	
		Assertions.assertThat(_appService.findByFirstname("User1")).isEqualTo(null);	
	}
	...
	public void findUsersByName_NameIsNotNullAndUsersExists_ReturnAUsers() {
		...
		Users users = mock(Users.class);
		Mockito.when(_usersRepository.findByFirstnameIgnoreCase(anyString())).thenReturn(users);
		Assertions.assertThat(_appService.findByFirstname("User1")).isEqualTo(_mapper.usersToFindUsersByFirstnameOutput(users));
	}
	@Test
